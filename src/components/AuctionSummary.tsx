...
// Interfaces for Player and Team
interface Player {
  id: number;
  sn: number;
  first_name: string;
...
export function AdminPanel({ isVisible, onClose }: AdminPanelProps) {
  ...
  const placeBid = async (teamName: string) => {
    ...
    const bidIncrement = (currentPlayer.category === 'C' || currentPlayer.category === 'B') ? 25000 : 50000;
  ...
  // *** MODIFIED FUNCTION ***
  // Accepts an optional price to override the currentBid from state
  const finalizeAuction = async (sold: boolean, winningTeam?: string, finalPrice?: number) => {
    if (!currentPlayer) return;

    const soldPrice = finalPrice ?? currentBid;

    try {
      if (sold && winningTeam) {
        await supabase.from('players').update({ status: 'sold', team_name: winningTeam, sold_price: soldPrice }).eq('id', currentPlayer.id);
        const team = teams.find(t => t.name === winningTeam);
        if (team) {
          const newPurse = team.remaining_purse - soldPrice;
        ...
        toast({ title: "Player Sold!", description: `${currentPlayer.first_name} ${currentPlayer.last_name} sold to ${teams.find(t => t.name === winningTeam)?.display_name} for NPR ${soldPrice.toLocaleString()}` });
      } else {
  ...
  // *** NEW FUNCTION ***
  // Handles the click on a team button, deciding whether to bid or finalize
  const handleTeamClick = (teamName: string) => {
    if (!currentPlayer) return;

    if (luckyDrawActive) {
      const finalPrice = currentBid > 0 ? currentBid : currentPlayer.base_price;
      finalizeAuction(true, teamName, finalPrice);
    } else {
      placeBid(teamName);
    }
  };
  ...
  if (loading) return <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center"><p className="text-white">Loading...</p></div>;
      ...
      <Card className="w-full max-w-6xl max-h-[90vh] overflow-y-auto">
        <CardHeader className="bg-primary text-primary-foreground">
          <div className="flex justify-between items-center">
            <CardTitle className="flex items-center gap-2"><Settings className="w-6 h-6" /> NPL Auction Admin Panel</CardTitle>
            <div className="flex items-center gap-2">
              <span className="text-sm">{user.email}</span>
              <Button variant="ghost" size="sm" onClick={handleLogout} className="text-primary-foreground hover:bg-primary-foreground/20"><LogOut className="w-4 h-4 mr-2" /> Logout</Button>
              <Button variant="ghost" size="sm" onClick={onClose} className="text-primary-foreground hover:bg-primary-foreground/20">✕</Button>
            </div>
          </div>
        </CardHeader>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                  {teams.map((team) => (
                    <Button
                      key={team.name}
                      variant="team"
                      className={cn("h-16 flex flex-col gap-1 text-sm", `hover:bg-${teamColors[team.name]}/20 hover:border-${teamColors[team.name]}`)}
                      onClick={() => handleTeamClick(team.name)} // *** UPDATED ONCLICK HANDLER ***
